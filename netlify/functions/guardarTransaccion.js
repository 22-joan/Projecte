...
// -------------------------
// Cargar datos de Google Sheets y enviarlos al backend
// -------------------------
async function cargarSARdeExcel() {
  ...
  try {
    ...
    estat.textContent = "✅ Datos de Sheets cargados y enviados al backend";
...
document.getElementById("form-transaccion").addEventListener("submit", (e) => {
  ...
  dadesSAR.push({ ...datos, sar, risc, pep: "No" });
  ...
  tr.innerHTML = `
    <td>${sar}</td>
    <td>${datos.nom}</td>
    <td>${datos.quantitat.toLocaleString()} €</td>
    <td>${risc}</td>
    <td>No</td>
  `;
  tbody.appendChild(tr);
